        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if ($categoriaActual == 1) {
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    ...
                    // Ejecutar la consulta para dar la activacion lógica al usuario
                    $queryBajaUsuario = "UPDATE usuarios SET Activo = 1 WHERE Usuario = '$usuarioSeleccionado'";
                    $resultBajaUsuario = mysqli_query($conexion, $queryBajaUsuario);

                    if ($resultBajaUsuario) {
                        // Éxito al activar el usuario
                        $exito = "El usuario \"$usuarioSeleccionado\" ha sido activado correctamente.";
                    } else {
                        // Error al activar al usuario
...
<head>
    ...
    <title>Resultado de la activacion de un Usuario</title>
    <style>
        body {
