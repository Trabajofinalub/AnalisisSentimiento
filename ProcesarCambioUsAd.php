        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if ($categoriaActual == 1) {
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    ...
                    $queryBajaUsuario = "UPDATE usuarios SET Categoria = 1 WHERE Usuario = '$usuarioSeleccionado'";
                    $resultBajaUsuario = mysqli_query($conexion, $queryBajaUsuario);

                    if ($resultBajaUsuario) {
                        // Éxito al dar de baja al usuario
                        $exito = "El usuario \"$usuarioSeleccionado\" cambió su Categoría a Administrador correctamente.";
                    } else {
                        // Error al dar de baja al usuario
                        $error = "Error al cambiar la Categoría del usuario.";
                    }
                }
            } else {
